...
import sys
sys.path.append("/pfs/work7/workspace/scratch/ma_sagnihot-projects/NAFNet")
...
def parse_options(is_train=True):
    ...
    parser.add_argument('--pretrain_network_g', type=str, required=False, default=None)
    parser.add_argument('--resume_state', type=str, required=False, default=None)
    
    parser.add_argument('--attack', type=str, required=False, default=None)
    parser.add_argument('--iterations', type=int, required=False, default=0)
    ...
    parser.add_argument('--folder', type=str, default="new_flc_attack", required=False, help='Path of output folder for testing')
    
    parser.add_argument('--upsampling_method', type=str, default='pixel', choices=['pixel', 'TransposedConv', 'FreqAvgUp'])
    parser.add_argument("-flc", "--flc_pooling", help="use flc_pooling while training", action="store_true")
    parser.add_argument("--use_alpha", help="flc_pooling with alpha blending while training", action="store_true")
    ...
    parser.add_argument("--half_precision", help="test_with_half_precision", action="store_true")
      
    parser.add_argument("--zero_padding", help="Use zero padding before FFT", action="store_true")
    
    
    parser.add_argument("-ga", "--gelu", help="use GELU in NAFNet rather than simple gate", action="store_true")
    parser.add_argument("-gaa", "--gelu2", help="use GELU with larger channels in NAFNet rather than simple gate", action="store_true")
    parser.add_argument("-ra", "--relu", help="use ReLU in NAFNet rather than simple gate", action="store_true")
    parser.add_argument("-lra", "--leaky_relu", help="use ReLU in NAFNet rather than simple gate", action="store_true")
    parser.add_argument("-at", "--adv_train", help="adversarial training", action="store_true")
    parser.add_argument("--attacking", help="adversarial attack", action="store_true")
    parser.add_argument("--blur", help="Use gaussian blur on high frequencies", action="store_true")
    parser.add_argument("--show_alpha", help="Show the learnt alpha value", action="store_true")

    parser.add_argument('--kernel_size', type=int, required=False, help="kernel size for transposed convolution for upsampling. <2 means use pixel shuffle", default=0)
    parser.add_argument('--para_kernel_size', type=int, required=False, help="parallel kernel size for transposed convolution for upsampling. <2 means use pixel shuffle", default=0)

    args = parser.parse_args()
    
    if args.zero_padding:
        args.padding = 'constant'
    else:
